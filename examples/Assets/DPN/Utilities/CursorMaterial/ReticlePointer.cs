...
namespace dpn
{
	...
	public class ReticlePointer : DpnBasePointer
	{
		...
		{
			transform.localPosition = reticleLocalPosition + transform.localPosition;
		...
		{
			reticleLocalPosition = transform.InverseTransformPoint(target);

			reticleLocalPosition.z =
				Mathf.Clamp(reticleLocalPosition.z + transform.localPosition.z, kReticleDistanceMin, kReticleDistanceMax)
